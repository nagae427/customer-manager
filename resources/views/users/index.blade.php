...
<div class="px-4 sm:px-6 lg:px-8 py-4"> 
    ...
    <div class="text-right mt-2 mb-4">
        @if(Auth::check() && Auth::user()->isAdmin())
            <button id="openEditModal" data-store-url="{{ route('users.store') }}" class="bg-blue-500 text-white font-bold py-2 px-4 rounded hover:bg-blue-600">新規登録</button>
        @endif
    </div>
        ...
        <table class="min-w-full text-left text-sm text-gray-700 border-collapse">
            ...
            <tbody>
                @foreach($users as $user)
                    ...
                    @endphp
                    {{-- ここを修正 --}}
                    <tr class="user-row hover:bg-gray-50 cursor-pointer" data-user='{{ $userData }}' data-user-id="{{ $user->id }}">
                        ...
                        <td class="actions js-no-modal-open px-4 py-2 border-b border-gray-200 whitespace-nowrap">
                            ...
                            @if(Auth::check() && Auth::user()->isAdmin())
                                {{-- 編集ボタンをモーダル表示用に変更 --}}
                                <button class="open-edit-modal text-green-600 hover:text-green-800 mr-2" data-store-url="{{ route('users.store') }}" data-user-id="{{ $user->id }}" title="営業担当者情報編集"><i class="fas fa-edit"></i></button>
                                {{-- 削除ボタンを追加 --}}
                                <button class="open-delete-modal text-red-600 hover:text-red-800" data-user-id="{{ $user->id }}" title="営業担当者削除"><i class="fas fa-trash"></i></button>
                            @endif
                        </td>
                    </tr>
                @endforeach
